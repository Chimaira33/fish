...
use crate::libc::{
    ...
    posix_spawn_file_actions_destroy, posix_spawn_file_actions_init, posix_spawnattr_destroy,
    posix_spawnattr_init, posix_spawnattr_setflags, posix_spawnattr_setpgroup,
    posix_spawnattr_setsigdefault, posix_spawnattr_setsigmask, PosixSpawnFileActionsT,
    PosixSpawnattrT, POSIX_SPAWN_SETPGROUP, POSIX_SPAWN_SETSIGDEF, POSIX_SPAWN_SETSIGMASK,
};
use crate::proc::Job;
...
struct Attr(PosixSpawnattrT);

impl Attr {
    fn new() -> Result<Self, Errno> {
        unsafe {
            let mut attr: PosixSpawnattrT = std::mem::zeroed();
...
struct FileActions(PosixSpawnFileActionsT);

impl FileActions {
    fn new() -> Result<Self, Errno> {
        unsafe {
            let mut actions: PosixSpawnFileActionsT = std::mem::zeroed();
    ...
    fn add_dup2(&mut self, src: libc::c_int, target: libc::c_int) -> Result<(), Errno> {
        unsafe { check_fail(posix_spawn_file_actions_adddup2(&mut self.0, src, target)) }
    }
}
