...
add_custom_target(
  sphinx-docs
  mkdir
  -p ${SPHINX_HTML_DIR}/_static/
  COMMAND
    ${SPHINX_EXECUTABLE} -j auto -q -b html -c "${SPHINX_SRC_DIR}" -d
    "${SPHINX_ROOT_DIR}/.doctrees-html" "${SPHINX_SRC_DIR}" "${SPHINX_HTML_DIR}"
...
add_custom_target(
  sphinx-manpages
  ${SPHINX_EXECUTABLE}
  -j auto -q -b man -c "${SPHINX_SRC_DIR}" -d "${SPHINX_ROOT_DIR}/.doctrees-man"
  "${SPHINX_SRC_DIR}"
...
if(BUILD_DOCS AND NOT SPHINX_EXECUTABLE)
  message(
    FATAL_ERROR
    "build documentation selected, but sphinx-build could not be found"
  )
endif()

if(
  IS_DIRECTORY ${CMAKE_CURRENT_SOURCE_DIR}/user_doc/html
...
elseif(HAVE_PREBUILT_DOCS)
  if(NOT CMAKE_CURRENT_SOURCE_DIR STREQUAL CMAKE_CURRENT_BINARY_DIR)
    ...
    add_custom_command(
      TARGET link_doc
      COMMAND
        ${CMAKE_COMMAND} -E create_symlink ${CMAKE_CURRENT_SOURCE_DIR}/user_doc
        ${CMAKE_CURRENT_BINARY_DIR}/user_doc
      POST_BUILD
    )
  endif()
endif()
