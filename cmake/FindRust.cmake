...
function(_findrust_version_ok ACTUAL_VERSION OUT_IS_OK)
  if(DEFINED Rust_FIND_VERSION_RANGE)
    ...
    elseif(Rust_FIND_VERSION_RANGE_MAX STREQUAL "EXCLUDE")
      ...
      message(
        FATAL_ERROR
        "Unexpected value in `<PackageName>_FIND_VERSION_RANGE_MAX`: "
  ...
  elseif(DEFINED Rust_FIND_VERSION)
    ...
    if(
      _TOOLCHAIN_${_TOOLCHAIN_SELECTED}_VERSION
      "${COMPARISON_OPERATOR}"
      Rust_FIND_VERSION
    )
      set("${OUT_IS_OK}" TRUE PARENT_SCOPE)
...
function(
  _corrosion_parse_target_triple
  target_triple
  out_arch
  out_vendor
  out_os
  ...
  if((NOT whole_match) AND (NOT CORROSION_NO_WARN_PARSE_TARGET_TRIPLE_FAILED))
    message(
      WARNING
      "Failed to parse target-triple `${target_triple}`."
  ...
  message(
    DEBUG
    "Parsed Target triple: arch: ${CMAKE_MATCH_1}, vendor: ${CMAKE_MATCH_3}, "
...
function(_corrosion_determine_libs_new target_triple out_libs)
  ...
  execute_process(
    COMMAND
      ${CMAKE_COMMAND} -E env "CARGO_BUILD_RUSTC=${Rust_COMPILER_CACHED}"
      ${Rust_CARGO_CACHED} rustc --verbose --color never
      --target=${target_triple} -- --print=native-static-libs
  ...
  if(cargo_build_result)
    message(
      DEBUG
      "Determining required native libraries - failed: ${cargo_build_result}."
    )
    ...
    if(cargo_build_error_message MATCHES "native-static-libs: ([^\r\n]+)\r?\n")
      ...
      list(
        TRANSFORM libs_list
        REPLACE "^msvcrt$" "\$<\$<CONFIG:Debug>:msvcrtd>"
      )
    else()
      message(
        DEBUG
        "Determining required native libraries - failed: Regex match failure."
      )
      message(
        DEBUG
        "`native-static-libs` not found in: `${cargo_build_error_message}`"
      )
      return()
    endif()
  endif()
...
if(NOT "${Rust_TOOLCHAIN}" STREQUAL "$CACHE{Rust_TOOLCHAIN}")
  ...
  set(
    ...
    ${Rust_TOOLCHAIN}
    CACHE STRING
    "Requested rustup toolchain"
    FORCE
  )
endif()

set(
  _RESOLVE_RUSTUP_TOOLCHAINS_DESC
...
if(DEFINED Rust_TOOLCHAIN)
  ...
  if(DEFINED Rust_COMPILER)
    ...
    if(
      NOT
        (
          EXISTS "${_Rust_COMPILER_TEST}"
          AND NOT IS_DIRECTORY "${_Rust_COMPILER_TEST}"
        )
    )
      set(
        _ERROR_MESSAGE
        "Rust_COMPILER was set to `${Rust_COMPILER}`, but this file does "
        "not exist."
      )
    ...
    if(NOT EXISTS "${_Rust_COMPILER_TEST}")
      set(
        _ERROR_MESSAGE
        "`rustc` not found in PATH or `$ENV{HOME}/.cargo/bin`.\n"
  ...
  execute_process(
    COMMAND
      ${CMAKE_COMMAND} -E env RUSTUP_FORCE_ARG0=rustup "${_Rust_COMPILER_TEST}"
      --version
...
if(Rust_RESOLVE_RUSTUP_TOOLCHAINS)
  execute_process(
    COMMAND "${Rust_RUSTUP}" toolchain list --verbose
  ...
  foreach(_TOOLCHAIN_RAW ${_TOOLCHAINS_RAW})
    if(
      ...
      if(
        ...
        set(
          _TOOLCHAIN_${_TOOLCHAIN}_VERSION
          "${CMAKE_MATCH_1}.${CMAKE_MATCH_2}.${CMAKE_MATCH_3}"
        )
        if(CMAKE_MATCH_4)
        ...
        message(
          AUTHOR_WARNING
          "Unexpected output from `rustc --version` for Toolchain `${_TOOLCHAIN}`: "
          "`${_TOOLCHAIN_RAW_VERSION}`.\n"
      ...
      message(
        AUTHOR_WARNING
        "Didn't recognize toolchain: ${_TOOLCHAIN_RAW}. Ignoring this toolchain.\n"
  ...
  if(
    ...
    message(
      FATAL_ERROR
      "Internal error - list length mismatch."
  ...
  set(
    ...
    CACHE INTERNAL
    "List of available Rustup toolchains"
    "${_DISCOVERED_TOOLCHAINS}"
  )
  set(
  ...
  set(
    Rust_RUSTUP_TOOLCHAINS_VERSION
    CACHE INTERNAL
  ...
  set(
    ...
    "${_TOOLCHAIN_SELECTED}"
    CACHE STRING
    "The rustup toolchain to use"
  )
  set_property(
    CACHE Rust_TOOLCHAIN
    PROPERTY STRINGS "${_DISCOVERED_TOOLCHAINS}"
  )
  ...
  if(NOT Rust_TOOLCHAIN IN_LIST _DISCOVERED_TOOLCHAINS)
    ...
    if(NOT "${Rust_TOOLCHAIN}-${_DEFAULT_HOST}" IN_LIST _DISCOVERED_TOOLCHAINS)
      set(
        _NOT_FOUND_MESSAGE
        "Could not find toolchain '${Rust_TOOLCHAIN}'\n"
...
if(NOT EXISTS "${Rust_COMPILER_CACHED}")
  set(
    _NOT_FOUND_MESSAGE
    "The rustc executable was not found. "
...
if(Rust_RESOLVE_RUSTUP_TOOLCHAINS)
  set(
    _NOT_FOUND_MESSAGE
    "Rust was detected to be managed by rustup, but failed to find `cargo` "
  ...
  set(
    _NOT_FOUND_MESSAGE
    "Failed to find `cargo` in PATH and `${_RUST_TOOLCHAIN_PATH}/bin`.\n"
  ...
  find_program(Rust_CARGO_CACHED cargo HINTS "${_RUST_TOOLCHAIN_PATH}/bin")
  # note: maybe can use find_package_handle_standard_args here, if we remove the _CACHED postfix.
  # not sure why that is here...
  if(NOT EXISTS "${Rust_CARGO_CACHED}")
    _findrust_failed(${_NOT_FOUND_MESSAGE})
...
if(_CARGO_VERSION_RAW MATCHES "cargo ([0-9]+)\\.([0-9]+)\\.([0-9]+)")
  ...
  set(
    Rust_CARGO_VERSION
    "${Rust_CARGO_VERSION_MAJOR}.${Rust_CARGO_VERSION_MINOR}.${Rust_CARGO_VERSION_PATCH}"
    CACHE INTERNAL
    ""
    FORCE
  )
...
if(_RUSTC_VERSION_RAW MATCHES "rustc ([0-9]+)\\.([0-9]+)\\.([0-9]+)(-nightly)?")
  ...
  set(
    Rust_VERSION
    "${Rust_VERSION_MAJOR}.${Rust_VERSION_MINOR}.${Rust_VERSION_PATCH}"
    CACHE INTERNAL
    ""
    FORCE
  )
...
if(_RUSTC_VERSION_RAW MATCHES "LLVM version: ([0-9]+)\\.([0-9]+)(\\.([0-9]+))?")
  ...
  set(
    Rust_LLVM_VERSION
    "${Rust_LLVM_VERSION_MAJOR}.${Rust_LLVM_VERSION_MINOR}.${Rust_LLVM_VERSION_PATCH}"
    CACHE INTERNAL
    ""
    FORCE
  )
...
if(NOT Rust_CARGO_TARGET_CACHED)
  ...
  if(WIN32)
    if(CMAKE_VS_PLATFORM_NAME)
      ...
      elseif("${LOWER_VS_PLATFORM_NAME}" STREQUAL "arm64")
        ...
        message(
          WARNING
          "VS Platform '${CMAKE_VS_PLATFORM_NAME}' not recognized"
        )
      endif()
    endif()
    # Fallback path
    if(NOT DEFINED _CARGO_ARCH)
      ...
      elseif(NOT "${CMAKE_SYSTEM_PROCESSOR}")
        message(
          WARNING
          "Failed to detect target architecture. Please set `CMAKE_SYSTEM_PROCESSOR`"
          " to your target architecture or set `Rust_CARGO_TARGET` to your cargo target triple."
        )
      else()
        message(
          WARNING
          "Failed to detect target architecture. Please set "
    ...
    elseif(
      NOT "${CMAKE_CROSSCOMPILING}"
      AND "${Rust_DEFAULT_HOST_TARGET}" MATCHES "-msvc$"
    )
      ...
      message(
        WARNING
        "Could not determine the target ABI. Please specify `Rust_CARGO_TARGET` manually."
      )
    endif()
    ...
    if(DEFINED _CARGO_ARCH AND DEFINED _CARGO_VENDOR AND DEFINED _CARGO_ABI)
      set(
        ...
        "${_CARGO_ARCH}-${_CARGO_VENDOR}-${_CARGO_ABI}"
        CACHE STRING
        "Target triple"
      )
    endif()
  ...
  if(NOT Rust_CARGO_TARGET_CACHED)
    if(CMAKE_CROSSCOMPILING)
      message(
        WARNING
        "CMake is in cross-compiling mode, but the cargo target-triple could not be inferred."
...
if(Rust_CARGO_TARGET_CACHED STREQUAL Rust_DEFAULT_HOST_TARGET)
  set(
    ...
    FALSE
    CACHE INTERNAL
    "Rust is configured for cross-compiling"
  )
else()
  set(
    Rust_CROSSCOMPILING
...
if(NOT DEFINED CACHE{Rust_CARGO_TARGET_LINK_NATIVE_LIBS})
  message(
    STATUS
    "Determining required link libraries for target ${Rust_CARGO_TARGET_CACHED}"
  )
  ...
  if(DEFINED required_native_libs)
    message(
      STATUS
      "Required static libs for target ${Rust_CARGO_TARGET_CACHED}: ${required_native_libs}"
    )
  endif()
...
if(
  Rust_CROSSCOMPILING
  AND NOT DEFINED CACHE{Rust_CARGO_HOST_TARGET_LINK_NATIVE_LIBS}
)
  message(
    STATUS
    "Determining required link libraries for target ${Rust_CARGO_HOST_TARGET_CACHED}"
  )
  unset(host_libs)
  ...
  if(DEFINED host_libs)
    message(
      STATUS
      "Required static libs for host target ${Rust_CARGO_HOST_TARGET_CACHED}: ${host_libs}"
    )
  endif()
  set(
    Rust_CARGO_HOST_TARGET_LINK_NATIVE_LIBS
    "${host_libs}"
    CACHE INTERNAL
