function __fish_complete_setxkbmap --description 'Complete setxkb options' --argument-names what
  sed -e "1,/! $what/d" -e '/^[[:space:]]*$/,$d' /data/data/com.termux/files/usr/share/X11/xkb/rules/xorg.lst | sed -r 's/[[:space:]]+([^[:space:]]+)[[:space:]]+(.+)/\1\t\2/'
end

set -l filter '"s/[^[:space:]]+[[:space:]][^[:space:]]+[[:space:]](.+)\((.+)\)/\1\t\2/;"'

complete -c setxkbmap -o '?' -o help -d 'Print this message'
complete -c setxkbmap -o compat -d 'Specifies compatibility map component name' -xa "(sed -r $filter /data/data/com.termux/files/usr/share/X11/xkb/compat.dir)"
...
complete -c setxkbmap -o geometry -d 'Specifies geometry component name' -xa "(sed -r $filter /data/data/com.termux/files/usr/share/X11/xkb/geometry.dir)"
complete -c setxkbmap -o I -d 'Add <dir> to list of directories to be used' -xa '(__fish_complete_directories)'
complete -c setxkbmap -o keycodes -d 'Specifies keycodes component name' -xa "(sed -r $filter /data/data/com.termux/files/usr/share/X11/xkb/keycodes.dir)"
complete -c setxkbmap -o keymap -d 'Specifies name of keymap to load' -xa "(sed -r $filter /data/data/com.termux/files/usr/share/X11/xkb/keymap.dir)"
...
complete -c setxkbmap -o symbols -d 'Specifies symbols component name' -xa "(sed -r $filter /data/data/com.termux/files/usr/share/X11/xkb/symbols.dir)"
complete -c setxkbmap -o synch -d 'Synchronize request w/X server'
complete -c setxkbmap -o types -d 'Specifies types component name' -xa "(sed -r $filter /data/data/com.termux/files/usr/share/X11/xkb/types.dir)"
